<x-app-layout>
    ...
    <div x-data="{ addModalOpen: @json($errors->any()), editModalOpen: false, editingUser: null, deleteModalOpen: false, editingUser: null, deletingUser: null }" class="py-2">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
                        ...
                        <form id="filter-form" method="GET" action="{{ route('admin.pengguna.index') }}" class="flex items-end gap-3">
                            <div>
                                <x-input-label for="search" value="Cari" />
                                <x-text-input id="search" type="text" name="search" value="{{ $search ?? '' }}" class="py-1" placeholder="Nama/Username" />
                            </div>
                        ...
                        <thead class="bg-gray-50">
                            @php
                                ...
                                $sortLink = function($col) use ($baseQuery, $sort_by, $sort_dir) {
                                    ...
                                    return route('admin.pengguna.index', array_merge($baseQuery, [
                                        'sort_by' => $col,
                                        'sort_dir' => $nextDir,
                                    ]));
                                };
                            @endphp
                            <tr>
                                @php $icons = $sortIcons('name'); @endphp
                                <th class="px-3 sm:px-6 py-2 sm:py-3">
                                    <a href="{{ $sortLink('name') }}" class="inline-flex items-center gap-1 group hover:text-indigo-500 transition duration-150 ease-in-out cursor-pointer py-1 px-2 rounded hover:bg-indigo-50">
                                ...
                                @php $icons = $sortIcons('username'); @endphp
                                <th class="px-3 sm:px-6 py-2 sm:py-3">
                                    <a href="{{ $sortLink('username') }}" class="inline-flex items-center gap-1 group hover:text-indigo-500 transition duration-150 ease-in-out cursor-pointer py-1 px-2 rounded hover:bg-indigo-50">
                                ...
                                @php $icons = $sortIcons('role'); @endphp
                                <th class="px-3 sm:px-6 py-2 sm:py-3">
                                    <a href="{{ $sortLink('role') }}" class="inline-flex items-center gap-1 group hover:text-indigo-500 transition duration-150 ease-in-out cursor-pointer py-1 px-2 rounded hover:bg-indigo-50">
                    ...
                    <div class="md:hidden mt-6 space-y-3">
                        <div class="flex items-end gap-2">
                            <form id="m-sort-form-user" method="GET" action="{{ route('admin.pengguna.index') }}" class="flex items-end gap-2">
                                ...
                                <div>
                                    ...
                                    <select id="sort_by_m" name="sort_by" class="border-gray-300 rounded-md shadow-sm py-1">
                                        ...
                                        <option value="username" @selected(($sortBy ?? '')==='username')>Username</option>
                            ...
                            <a href="{{ route('admin.pengguna.index', ['search' => $search ?? null, 'sort_by' => $sortBy ?? 'name', 'sort_dir' => ($sortDir ?? 'asc')==='asc' ? 'desc' : 'asc']) }}" class="inline-flex items-center px-3 py-2 bg-indigo-600 text-white rounded-md text-xs">
                                Arah: {{ ($sortDir ?? 'asc')==='asc' ? 'Naik' : 'Turun' }}
                            </a>
                        </div>
                        <script>
                            document.addEventListener('DOMContentLoaded', () => {
                                const form = document.getElementById('m-sort-form-user');
